...
void eventDetectionMenu(SDL_Window* pWindow){
    // Interaction section
    ...
    while (SDL_PollEvent(&event)) // Getting user's inputs
    {
      switch(event.type)
      {
          ...
          case SDL_KEYUP: // Key relaxing
              if (event.key.keysym.sym == SDLK_1) // 1 key relaxed, launches a new game
              {
                launchGame(pWindow);
              }
              break;
              if (event.key.keysym.sym == SDLK_2) // 2 key relaxed, launches saves selection menu
              {
                // Créer menu
              }
              if (event.key.keysym.sym == SDLK_3) // 3 key relaxed, launches parameters menu
              {
                // Créer menu
              }
              if (event.key.keysym.sym == SDLK_4) // 4 key realxed, exits game
              {
...
// Plus utile, peut-être à supprimer
